...
Cpu::InstructionType Cpu::runNextInstuction()
{
	...
	if (status == Instruction::INSTRUCTION_STATUS_UNHANDLED_FETCH || status == Instruction::INSTRUCTION_STATUS_UNALIGNED_ACCESS)
	{
	...
	InstructionType result = Cpu::decodeAndExecute(instruction);
...
{
	// Fixme
	RegisterIndex registerDestinationIndex = instruction.getRegisterDestinationIndex();
	RegisterIndex registerTargetIndex = instruction.getRegisterTargetIndex();
	uint32_t shiftImmVal = instruction.getShiftImmediateValue();
	uint32_t resultVal = getRegisterValue(registerTargetIndex) << shiftImmVal;
	setRegisterValue(registerDestinationIndex,  resultVal);
...
{
	// Fixme
	uint32_t jumpValue = instruction.getJumpTargetValue();
	m_pc = (m_pc & 0xf0000000) | (jumpValue << 2);
...
Cpu::InstructionType Cpu::opcodeBNE(const Instruction& instruction)
{
	...
	RegisterIndex registerSourceIndex = instruction.getRegisterSourceIndex();
	RegisterIndex registerTargetIndex = instruction.getRegisterTargetIndex();
	if (getRegisterValue(registerSourceIndex) != getRegisterValue(registerTargetIndex))
	{
		branch(signExtendedValue, m_pc);
	}
...
Cpu::InstructionType Cpu::opcodeLW(const Instruction& instruction)
{
	...
	m_load = RegisterData(registerTargetIndex, value.getInstructionOpcode());
...
Cpu::InstructionType Cpu::opcodeSH(const Instruction& instruction)
{
	...
	RegisterIndex registerTargetIndex = instruction.getRegisterTargetIndex();
	uint32_t addr = getRegisterValue(registerSourceIndex) + signExtendedValue;
	uint16_t value = (uint16_t) getRegisterValue(registerTargetIndex);
	store16(addr, value);
...
{
	// Fixme
	uint32_t returnAddr = m_pc;
	setRegisterValue(RegisterIndex(31), returnAddr);
	opcodeJ(instruction);
...
Cpu::InstructionType Cpu::opcodeSB(const Instruction& instruction)
{
	...
	RegisterIndex registerTargetIndex = instruction.getRegisterTargetIndex();
	uint32_t addr = getRegisterValue(registerSourceIndex) + signExtendedValue;
	uint8_t value = (uint8_t)getRegisterValue(registerTargetIndex);
	store8(addr, value);
...
Cpu::InstructionType Cpu::opcodeJR(const Instruction& instruction)
{
	// Fixme
	RegisterIndex registerSourceIndex = instruction.getRegisterSourceIndex();
	m_pc = getRegisterValue(registerSourceIndex);
